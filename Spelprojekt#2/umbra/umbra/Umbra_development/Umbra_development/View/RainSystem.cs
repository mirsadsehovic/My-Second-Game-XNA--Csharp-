...
namespace Umbra_development.View
{
   public class RainSystem
    {
         private const int NUM_OF_PARTICLES = 1000;
        private const float DROP_SPEED = 7.0f;
        private readonly Random m_rand; 

        RainDropParticle[] m_raindrops = new RainDropParticle[NUM_OF_PARTICLES];
        ...
        private RainDropParticle GenerateRainDrop()
        {
            ...
            return new RainDropParticle(position, velocity, lifetime);
        }
        ...
        internal void DrawRainDrops(SpriteBatch a_spriteBatch, Texture2D a_texture, float a_elapsedTime)
        {
            for (int i = 0; i < NUM_OF_PARTICLES; i++ )
            {
